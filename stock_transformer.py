...
class TransformerModel(nn.Module):
    def __init__(self, input_size, d_model, nhead, num_encoder_layers, dim_feedforward, dropout_p, output_size):
        super(TransformerModel, self).__init__()
        self.model_type = 'Transformer'

        self.input_layer = nn.Linear(input_size, d_model)
        self.positional_encoding = nn.Parameter(torch.zeros(1, 1000, d_model))
        encoder_layers = nn.TransformerEncoderLayer(d_model, nhead, dim_feedforward, dropout_p, batch_first=True)
        self.transformer_encoder = nn.TransformerEncoder(encoder_layers, num_encoder_layers)
        self.fc_out = nn.Linear(d_model, output_size)

    def forward(self, x):
        x = self.input_layer(x) + self.positional_encoding[:, :x.size(1), :]
...
model = TransformerModel(input_size, d_model, nhead, num_encoder_layers, dim_feedforward, dropout_p, output_size)
...
def train_model(model, early_stop, n_epochs, progress_interval, train_batches, val_batches):
    ...
    for epoch in range(n_epochs):
        ...
        if (epoch % progress_interval) == 0:
            print(f"Epoch {epoch}: Train Loss = {train_losses[-1]:.4f}, Validation Loss = {valid_losses[-1]:.4f}")
...
def predict(symbol):
    ...
    predict = pd.DataFrame(y_scaler.inverse_transform(np.array(y_test_preds)), columns=['Transformer 예측'])
    original = pd.DataFrame(y_scaler.inverse_transform(np.array(y_tests)), columns=['실제값'])
    predict['Time Index'] = range(len(predict))
    original['Time Index'] = range(len(original))

    RMSE = mean_squared_error(original['실제값'], predict['Transformer 예측']) ** 0.5
    print(f'RMSE for {symbol}: {RMSE}')

    MDA = calculate_mda(original['실제값'].values, predict['Transformer 예측'].values)
    print(f'MDA for {symbol}: {MDA:.2f}')

    result = pd.merge(original, predict, on='Time Index')
    result.to_csv(f"{symbol}_transformer_predictions.csv", index=False)
    print(f"Predictions saved to {symbol}_transformer_predictions.csv")
