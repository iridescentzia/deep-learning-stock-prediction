...
def predict(symbol):
    ...
    predict = pd.DataFrame(y_scaler.inverse_transform(np.array(y_test_preds)), columns=['MLP 예측'])
    ...
    # RMSE
    RMSE = mean_squared_error(original['실제값'], predict['MLP 예측']) ** 0.5
    print(f'RMSE for {symbol}: {RMSE}')

    # MDA
    MDA = calculate_mda(original['실제값'].values, predict['MLP 예측'].values)
    print(f'MDA for {symbol}: {MDA:.2f}')

    result = pd.merge(original, predict, on='Time Index')
    result.to_csv(f"{symbol}_mlp_predictions.csv", index=False)
    print(f"Predictions saved to {symbol}_mlp_predictions.csv")
